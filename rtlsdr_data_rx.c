...
int main(){
    ...
    short *frame_moc = malloc((bsize*rate_upscale)*sizeof(short));

    struct gfsk_demod* mod = create_gfsk_demod(8000.0,8000,64000);
    ...
    while(1){
      ...
      short* framet = frame_moc;
      for(int i = 0;i<bsize;i++){
          short point = frame[i];
          for(int i = 0;i<rate_upscale;i++){
            *framet = (point+prev)/2;
            prev = point;
            framet++;
          }
      ...
      int chars = run_gfsk_demod(mod,frame_moc,output,bsize*rate_upscale);
